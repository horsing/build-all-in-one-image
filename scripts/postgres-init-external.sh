...
RETRIES=5
until PGPASSWORD="${ILLA_PG_PASSWORD:-postgres}" psql \
  -h "${ILLA_PG_ADDR:-127.0.0.1}" -p "${ILLA_PG_PORT:-5432}" \
  -U "${ILLA_PG_USER:-postgres}" "${ILLA_PG_DATABASE:-postgres}" \
  -c "select 1" > /dev/null 2>&1 || [ $RETRIES -eq 0 ]; do
...
echo 
PGPASSWORD="${ILLA_PG_PASSWORD:-postgres}" psql \
  -h "${ILLA_PG_ADDR:-127.0.0.1}" -p "${ILLA_PG_PORT:-5432}" \
  -U "${ILLA_PG_USER:-postgres}" "${ILLA_PG_DATABASE:-postgres}" <<EOF

-- init ${ILLA_PG_DATABASE:-illa_builder}


-- create database ${ILLA_PG_DATABASE:-illa_builder};

\c ${ILLA_PG_DATABASE:-illa_builder};

-- create user ${ILLA_PG_USER:-illa_builder} with encrypted password 'illa2022';

-- grant all privileges on database ${ILLA_PG_DATABASE:-illa_builder} to ${ILLA_PG_USER:-illa_builder};
